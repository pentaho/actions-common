...
| `run_register_ram_plugin_release` | boolean | No       | `false`                        | When `true`, triggers the `register-ram-plugin-version.yml` workflow to register the promoted artifact in RAM as a new plugin version. Note that this requires the presence of a plugin-details.yaml file in the repository.                   |
